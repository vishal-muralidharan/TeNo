...
const sortRegex = 
  \.sort\(\(a, b\) => {\s*if \(a\\.key === 'favorites'\) return -1;\s*if \(b\\.key === 'favorites'\) return 1;\s*if \(a\\.key === 'ungrouped'\) return 1;\s*if \(b\\.key === 'ungrouped'\) return -1;\s*return a\\.label\\.localeCompare\(b\\.label\);\s*}\);/;
